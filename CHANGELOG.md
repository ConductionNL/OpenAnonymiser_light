...
### Fixed
- **K8s CPU-threadpinning** (`k8s/base/deployment.yaml`): `OMP_/MKL_/OPENBLAS_/NUMEXPR_NUM_THREADS=6` + `TOKENIZERS_PARALLELISM=false`, gematcht aan `limits.cpu`. torch/OpenMP las `os.cpu_count()` = node-cores (32) i.p.v. de cgroup-quota (6) → 32 threads op 6 cores = oversubscription + CFS-throttling. Gemeten effect (dev, CPU): 1,3k tekens **23-30s → 2,0s**, 2,6k **48s+error → 3,8s**; grote docs die eerst de ingress-timeout (504) raakten voltooien nu. Geen GPU nodig voor normale tekstgroottes.
...
- CI: test-API start nu met `PLUGINS_CONFIG=src/api/plugins.classic.yaml` in zowel `feature-testing.yml` als `docker-build.yml` test-job. Voorheen draaide de API tegen default `plugins.yaml` (alle regex disabled, legacy state) — pytest skipte 29 tests met "entity type wordt niet ondersteund". Met classic-config zijn alle pattern-recognizers actief en geeft de suite echte coverage van BSN/IBAN/e-mail/passport/etc.
- **Fix: SpacyRecognizer entities-lijst** in alle drie de plugin-yaml's (`plugins.yaml`, `plugins.classic.yaml`, `plugins.gpu.yaml`): raw spaCy types (`LOC`, `GPE`, `ORG`) vervangen door presidio-side namen (`LOCATION`, `ORGANIZATION`). Anders crashte `POST /api/v1/analyze` met `entities: ["LOCATION"]` op `500 — No matching recognizers were found`. Mapping LOC/GPE→LOCATION + ORG→ORGANIZATION in `plugin_loader.py:144` deed alleen de output-naming, niet de SUPPORTED_ENTITIES-registratie. Test-suite (`test_location_detected`, `test_organization_detected`) faalde daar ook op.
- **CVE-fix wheel-metadata** in beide Dockerfiles (`Dockerfile.classic`, `Dockerfile.gpu`): `pip install --upgrade --no-deps "jaraco.context>=6.1.0" "wheel>=0.46.2"` na `uv sync`. Trivy meldde HIGH-CVE's (CVE-2026-23949 path traversal, CVE-2026-24049 priv-esc) op de wheel-metadata die niet via uv.lock te beheren is.
- **`.trivyignore` met sunset** voor 8 OpenSSL OS-CVE's (CVE-2025-15467, CVE-2025-69419/69421, CVE-2026-28387/28388/28389/28390/31789). Patches landen via Debian-security-archive maar zitten nog niet in onze base-image; `apt-get dist-upgrade` pikt ze op zodra het mirror geïndexeerd is. Sunset 2026-05-17 — review verplicht.
